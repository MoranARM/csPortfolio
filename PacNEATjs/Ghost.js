class Ghost{
  constructor(pac){
    this.gpos = tileToPixel(createVector(13, 11));//createVector(13*16+8, 11*16+8);//starting position of the ghost
  ...
  setNodes(){
    ...
    this.ghostNodes.push(new PathNode(pixelToTile(this.gpos)));
    for(let i=1; i<27; i++){
      for(let j=1; j<30; j++){
        if(!originalTiles[j][i].wall){//if there is a space up or down and left or right then it is a node
          if(!originalTiles[j-1][i].wall || !originalTiles[j+1][i].wall){//check up or down
            if(!originalTiles[j][i-1].wall || !originalTiles[j][i+1].wall){//check left or right
              this.ghostNodes.push(new PathNode(i, j));//add the nodes
            }
          }
        }
      }
    }if(this.goHome){//if retruning home then the target is above the ghost spawn
      this.ghostNodes.push(new PathNode(13, 11));
    }else{
      if(this.chase){
        if(this.clyd){//true if ghost is orange
          if(dist((this.gpos.x-8)/16, (this.gpos.y-8)/16, (pacman.pos.x-8)/16, (pacman.pos.y-8)/16)>8){
            //this.ghostNodes.push(new PathNode((pacman.pos.x-8)/16, (pacman.pos.y-8)/16));//go after pacman
            this.ghostNodes.push(new PathNode(pixelToTile(this.pacman.pos)));
          }else{
            //this.ghostNodes[this.ghostNodes.length] = new PathNode(this.scatx, this.scaty);//run away from pacman
            this.ghostNodes.push(new PathNode(this.scatx, this.scaty));
          }
          ...
          //this.ghostNodes.push(new PathNode((pacman.pos.x-8)/16, (pacman.pos.y-8)/16));
          this.ghostNodes.push(new PathNode(pixelToTile(pacman.pos)));
        }
      }else{
        this.ghostNodes.push(new PathNode(this.scatx, this.scaty));//scatter to corner
      }
  ...
  checkDirection(){
    if(pacman.hitPacman(this.gpos)){//if pacman gets hit
      ...
      let tilePos = pixelToTile(this.gpos);
      ...
      if(originalTiles[floor(arrPos.y)][floor(arrPos.x)].tunnel){//checks if the next position will be in the tunnel
        ...
        for(let j=0; j<this.ghostNodes.length; j++){
          ...
          if(replay && upToIndex<frightenedTurns.length){
            rand = frightenedTurns[upToIndex];
            upToIndex++;
          }
          switch(rand){
          case 0:
          ...
          if(!replay){
            this.frightenedTurns.push(rand);
          }
        }
      }else{//not frightened
        this.setPath();
